...
const EChartsLineDynamicSlidingThresholdPage = () => {
  const [chartData, setChartData] = useState({
    categories: [],
    series: [
      { name: '진입 차량 수', data: [], color: 'red' },
      { name: '진출 차량 수', data: [], color: 'blue' },
    ]
  });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // ✅ 임계값 목록
      const thresholds = [
        { name: 'X Danger High', value: 30, direction: 'over' },  // 초과 경고
        { name: 'X Danger Low', value: 10, direction: 'under' },  // 미만 경고
        { name: 'Y Danger High', value: 35, direction: 'over' },  // 초과 경고
        { name: 'Y Danger Low', value: 15, direction: 'under' },  // 미만 경고
      ];
      ...
      thresholds.forEach((t) => {
        ...
        if (t.name.startsWith('X')) {
          // 진입 차량 수 검사
          if (t.direction === 'over' && newEntry > t.value) {
            console.warn(`%c${label} 진입 차량 수 ${newEntry} > ${t.value}`, 'color: red; font-weight: bold;');
          }
          if (t.direction === 'under' && newEntry < t.value) {
            console.warn(`%c${label} 진입 차량 수 ${newEntry} < ${t.value}`, 'color: red; font-weight: bold;');
          }
        }
        ...
        if (t.name.startsWith('Y')) {
          // 진출 차량 수 검사
          if (t.direction === 'over' && newExit > t.value) {
            console.warn(`%c${label} 진출 차량 수 ${newExit} > ${t.value}`, 'color: blue; font-weight: bold;');
          }
          if (t.direction === 'under' && newExit < t.value) {
            console.warn(`%c${label} 진출 차량 수 ${newExit} < ${t.value}`, 'color: blue; font-weight: bold;');
          }
        }
    });
  ...
  const handleWindowSizeChange = (e) => {
    ...
    <div className="page-container">
      ...
      <EChartsLineChart
        ...
        sliding={isSliding}
        thresholds={[
            { name: 'X Danger', value: 30, color: 'red' ,lineType: 'dashed'},
            { name: 'X Danger', value: 10, color: 'red' ,lineType: 'dashed'},
            { name: 'Y Danger', value: 35, color: 'blue',lineType: 'dashed' },
            { name: 'Y Danger', value: 15, color: 'blue',lineType: 'dashed'},
        ]}
      />
